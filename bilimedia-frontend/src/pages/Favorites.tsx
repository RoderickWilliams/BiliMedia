...
export default function Favorites() {
  ...
  if (!isLoggedIn || !user) {
    return (
      <div className="mx-auto max-w-xl py-20 text-center text-[color:var(--color-txt-2)]">
        ...
        <div className="text-[14px]">登录后才能查看收藏（多入口同步）</div>
      </div>
    );
  }
  ...
  const handleDelete = async (id: string) => {
    setBusyDel(id);
    try { await removeFavorite(user.id, id); }
  ...
  return (
    <div className="mx-auto w-full max-w-[1100px] px-5 sm:px-8 py-8">
      <div className="mb-6">
        ...
        <p className="text-[14px] text-[color:var(--color-txt-2)] mt-1">管理你收藏的视频和音乐（多入口同步）</p>
      </div>
          ...
          {filtered.filter(i => i.type === 'video').map((item) => (
            <div key={item.id} className="card overflow-hidden group">
              <div className="relative aspect-video bg-gray-200">
                ...
                <button
                  className={`absolute top-2 right-2 p-1.5 bg-white/90 rounded-lg text-pink-500 hover:bg-white transition opacity-0 group-hover:opacity-100 ${busyDel === item.id ? 'opacity-40' : ''}`}
                  onClick={() => setConfirmDelete(item.id)}
                  disabled={busyDel === item.id}
                >
      ...
      {tab === 'all' && items.filter(i => i.type === 'music').length > 0 && (
        <div className="mb-2 mt-4 text-[12px] font-medium text-gray-500 px-1">音乐</div>
      )}
      {tab !== 'video' && (
          ...
          {filtered.filter(i => i.type === 'music').map((item) => (
            <div key={item.id} className="card p-3 flex items-center gap-4">
              ...
              <div className="flex items-center gap-1.5">
                ...
                <button
                  className={`p-2 rounded-lg hover:bg-red-50 text-pink-500 ${busyDel === item.id ? 'opacity-40' : ''}`}
                  onClick={() => setConfirmDelete(item.id)}
                  disabled={busyDel === item.id}
                >
      ...
      {confirmDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-2xl shadow-xl w-[360px] p-6 fade-up">
            ...
            <p className="text-[13.5px] text-gray-500 mb-5">确定要取消收藏吗？（其它入口也会同步移除）</p>
